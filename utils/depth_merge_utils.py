...
def get_camera_parameters(cam: CameraInfo) -> Tuple[np.ndarray, np.ndarray]: # (intrinsics, extrinsics)
    
    extrinsics = np.block([[cam.R, cam.T.reshape(3, 1)], [np.zeros((1, 3)), 1]])

    focal_length_y = fov2focal(cam.FovY, cam.height)
    focal_length_x = fov2focal(cam.FovX, cam.width)


    intrinsics = np.array([[focal_length_x, 0, cam.width/2],
...
def filter_depths(cam_infos: List[CameraInfo], depth_est: List[torch.Tensor]):
    ...
    for ref_view_idx, src_views_idx in pair_data:
        # load the camera parameters
        ref_intrinsics, ref_extrinsics = get_camera_parameters(cam_infos[ref_view])
