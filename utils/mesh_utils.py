...
from utils.render_utils import save_img_f32, save_img_u8, save_depthmap
...
class GaussianExtractor(object):
    ...
    def reconstruction_optim(self, gaussians, viewpoint_stack, optim_test_pose_iter, pipeline, background):
        """
        reconstruct radiance field given cameras
        """
        self.clean()
        ...
        # for idx, view in enumerate(tqdm(self.viewpoint_stack, desc="Rendering progress")):
        for idx, view in tqdm(enumerate(self.viewpoint_stack), desc="reconstruct radiance fields"):
            ...
            print("opt_pose-camera_pose: ", optimal_pose-camera_pose)

            render_pkg = render(view, gaussians, pipeline, background, camera_pose=optimal_pose)
            rgb = render_pkg['render']
            alpha = render_pkg['rend_alpha']
            normal = torch.nn.functional.normalize(render_pkg['rend_normal'], dim=0)
            depth = render_pkg['surf_depth']
            ...
            self.normals.append(normal.detach().cpu())
    ...
    def export_image(self, path, ds_path=None):
        ...
        render_normal_path = os.path.join(path, "render_normal")
        surf_nomal_path = os.path.join(path, "surf_normal")
